...
file_prefix = "Deciduous_Forest"

# Random sampling settings
use_random_sample = True  # Set to False to process all features
max_samples = 5000  # Maximum number of random samples to process (only used if use_random_sample = True)
...
# Get all features and optionally sample them
all_features = list(vector_layer.getFeatures())
total_available = len(all_features)

if use_random_sample and total_available > max_samples:
    print(f"Randomly sampling {max_samples} features from {total_available} total features")
    features_to_process = random.sample(all_features, max_samples)
else:
    print(f"Processing all {total_available} features")
